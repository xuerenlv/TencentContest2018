...
def train_eval_model(graph_hyper_params):
    ...
    ad_data['prod_fea'] = ad_data['prod_fea'].apply(lambda x: [float(i) for i in x[1:-1].split(',')])

    ad_features_engineer_list = []
    fes_size = len(list(ad_data.iloc[0]['prod_fea']))
    print 'added_feature_size:', fes_size
    for i in range(feature_conf_dict['aid']):
        ut = ad_data[ad_data['aid'] == i]
        if len(ut) == 0:
            print 'aid_miss:', i
            ad_features_engineer_list.append(list(np.random.randn(fes_size)))
        else:
            ad_features_engineer_list.append(ut.iloc[0]['prod_fea'])
    ad_features_engineer_list = np.array(ad_features_engineer_list).astype(np.float32)
    # print ad_features_engineer_list
    # import sys
    # sys.exit(0)
    ...
    with graph.as_default():
        ...
        pred_val, model_loss, network_params = inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p,
                                                         ...
                                                         graph_hyper_params, ad_features_engineer_list, fes_size)
