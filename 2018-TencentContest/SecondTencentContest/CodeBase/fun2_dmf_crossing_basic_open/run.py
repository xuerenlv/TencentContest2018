    ...
    def __call__(self, vda, interest=None, feature_config=None):
        ...
        if interest is not None:
            if interest not in self.split_cache_interest:
                self.split_cache_interest[interest] = {}
                self.split_cache_rem_size_interest[interest] = {}

            for d in vda:
                if type(d) != str or '|' not in d:
                    if interest not in self.split_cache_interest[interest]:
                        ind, val = [0] * feature_config[interest][0], [-1.0] * feature_config[interest][0]
                        index_data.append(ind)
                        val_data.append(val)
                        self.split_cache_interest[interest][interest] = (ind, val)
                        self.split_cache_rem_size_interest[interest][interest] = 1
                    else:
                        index_data.append(self.split_cache_interest[interest][interest][0])
                        val_data.append(self.split_cache_interest[interest][interest][1])
                        self.split_cache_rem_size_interest[interest][interest] += 1
                else:
                    if d not in self.split_cache_interest[interest]:
                        ind, val = [0] * feature_config[interest][0], [-1.0] * feature_config[interest][0]
                        dl = d.split('|')
                        ...
                        index_data.append(ind)
                        val_data.append(val)
                        # self.split_cache_interest[interest][d] = (ind, val)
                        # self.split_cache_rem_size_interest[interest][d] = 1
                    else:
                        index_data.append(self.split_cache_interest[interest][d][0])
                        val_data.append(self.split_cache_interest[interest][d][1])
                        self.split_cache_rem_size_interest[interest][d] += 1
            # print 'index:', np.array(index_data).shape, interest, feature_config[interest][0]
            # print 'val:', np.array(val_data).shape, interest
...
def train_eval_model(graph_hyper_params):
    ...
    with graph.as_default():
        ...
        topic1_index_p = tf.placeholder(tf.int32, [None, feature_conf_dict['topic1'][0]], name="topic1_index")
        topic1_val_p = tf.placeholder(tf.float32, [None, 1, feature_conf_dict['topic1'][0]], name="topic1_val")
        topic2_index_p = tf.placeholder(tf.int32, [None, feature_conf_dict['topic2'][0]], name="topic2_index")
        topic2_val_p = tf.placeholder(tf.float32, [None, 1, feature_conf_dict['topic2'][0]], name="topic2_val")
        topic3_index_p = tf.placeholder(tf.int32, [None, feature_conf_dict['topic3'][0]], name="topic3_index")
        topic3_val_p = tf.placeholder(tf.float32, [None, 1, feature_conf_dict['topic3'][0]], name="topic3_val")
        ...
        def get_fed_dict(b_data, split_vector_data, feature_conf_dict, predict=False):
            ...
            # print 124
            topic1_li = split_vector_data(b_u_d['topic1'], interest='topic1', feature_config=feature_conf_dict)
            fed_dict[topic1_index_p], fed_dict[topic1_val_p] = topic1_li[0], topic1_li[1]
            topic2_li = split_vector_data(b_u_d['topic2'], interest='topic2', feature_config=feature_conf_dict)
            fed_dict[topic2_index_p], fed_dict[topic2_val_p] = topic2_li[0], topic2_li[1]
            topic3_li = split_vector_data(b_u_d['topic3'], interest='topic3', feature_config=feature_conf_dict)
