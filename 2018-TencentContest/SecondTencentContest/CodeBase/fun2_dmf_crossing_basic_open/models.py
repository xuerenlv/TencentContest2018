...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        topic1_v_det = tf.reshape(topic1_v_ori_prod, [-1, feature_conf_dict['topic1'][0] * emb_size])
        ...
        topic2_v_det = tf.reshape(topic2_v_ori_prod, [-1, feature_conf_dict['topic2'][0] * emb_size])
        ...
        topic3_v_det = tf.reshape(topic3_v_ori_prod, [-1, feature_conf_dict['topic3'][0] * emb_size])
        ...
        user_vector = tf.concat([lbs_v, age_v, carrier_v, consumptionability_v, education_v, gender_v, house_v, os_v, ct_v,
                                ...
                                interest4_v_det, interest5_v_det, kw1_v, kw2_v, kw3_v, topic1_v_det, topic2_v_det, topic3_v_det], axis=-1)

        user_vector_size = emb_size * 10 + 4 * low_emb_size + (feature_conf_dict['marriageStatus'][0]+ \
                           feature_conf_dict['interest1'][0] + \
                           feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0] \
                           +feature_conf_dict['interest5'][0]+  + feature_conf_dict['topic1'][0] + feature_conf_dict['topic2'][0] + feature_conf_dict['topic3'][0]) * emb_size
        ...
        if graph_hyper_params['model'] == 'dmf':
            print 'dmf model !'
            user_vector = tf.reshape(user_vector, [-1, 10 + 4 + (feature_conf_dict['marriageStatus'][0] + \
                        feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] +\
                        feature_conf_dict['interest4'][0] + feature_conf_dict['interest5'][0]+ \
                        feature_conf_dict['topic1'][0] + feature_conf_dict['topic2'][0] + feature_conf_dict['topic3'][0]), emb_size])
            ad_vector = tf.reshape(ad_vector, [-1, 7, ad_emb_size])

            relation_mat = tf.matmul(user_vector, tf.transpose(ad_vector, perm=[0, 2, 1]))

            uu_s = 10 + 4 + feature_conf_dict['marriageStatus'][0] + feature_conf_dict['interest1'][0] + \
                   feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0] \
                   + feature_conf_dict['interest5'][0] + feature_conf_dict['topic1'][0] + feature_conf_dict['topic2'][0] + feature_conf_dict['topic3'][0]
            ad_s = 7
            flat_size = uu_s * ad_s
