...
# from models_deepfm import inference
# from models_croos_field import inference
...
def train_eval_model(graph_hyper_params):
    ...
    with graph.as_default():
        ...
        # clu_500_p = tf.placeholder(tf.int32, [None, 1], name="clu_500_p")
        clu_1000_p = tf.placeholder(tf.int32, [None, 1], name="clu_1000_p")
        ...
        def get_fed_dict(b_data, split_vector_data, feature_conf_dict, predict=False):
            ...
            # fed_dict[clu_200_p] = np.expand_dims(b_u_d['cl200'], axis=1)
            # fed_dict[clu_500_p] = np.expand_dims(b_u_d['cl500'], axis=1)
            fed_dict[clu_1000_p] = np.expand_dims(b_u_d['cl1000'], axis=1)
