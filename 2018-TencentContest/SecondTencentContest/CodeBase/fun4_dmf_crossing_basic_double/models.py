...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    def gen_basic_relation_mat(name):
        ...
        return relation_mat_1, user_vector, ad_vector
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            umt2, u2v, v2v = gen_basic_relation_mat("SecondOutAll")
            relation_mat_c1 = tf.reshape(tf.matmul(u2v, tf.transpose(ad_vector, perm=[0, 2, 1])), [-1, flat_size])
            relation_mat_c2 = tf.reshape(tf.matmul(user_vector, tf.transpose(v2v, perm=[0, 2, 1])), [-1, flat_size])

            final_relation_mat = tf.concat([relation_mat, umt2, relation_mat_c1, relation_mat_c2], axis=-1)
            flat_size *= 4
            final_size = 2000
