...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            # ad_vector = tf.concat(
            #     [tf.tile(advertiserid_v, [1, uu_s]),
            #      tf.tile(campaignid_v, [1, uu_s]),
            #      tf.tile(creativeid_v, [1, uu_s]),
            #      tf.tile(adcategoryid_v, [1, uu_s]),
            #      tf.tile(productid_v, [1, uu_s]),
            #      tf.tile(producttype_v, [1, uu_s]),
            #      tf.tile(creativesize_v [1, uu_s])], axis=-1)
            # ad_vector = tf.reshape(ad_vector, [-1, uu_s * 7, ad_emb_size])
            # relation_mat = tf.concat([user_vector_ss, ad_vector], axis=-1)
            ...
            wf_1 = tf.get_variable("wf_1", shape=(flat_size, final_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_1 = tf.get_variable("bf_1", shape=[final_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_pre_1 = tf.matmul(relation_mat, wf_1) + bf_1
            final_vec_wf_1 = tf.nn.relu(final_vec_wf_pre_1) + relation_mat

            wf_2 = tf.get_variable("wf_2", shape=(final_size, final_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_2 = tf.get_variable("bf_2", shape=[final_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_2 = tf.matmul(final_vec_wf_1, wf_2) + bf_2
            final_vec_wf_2 = tf.nn.relu(final_vec_wf_2) + final_vec_wf_1

            wf_3 = tf.get_variable("wf_3", shape=(final_size, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
