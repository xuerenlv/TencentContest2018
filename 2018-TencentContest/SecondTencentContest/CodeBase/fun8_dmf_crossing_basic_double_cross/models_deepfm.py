...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        marriageStatus_emb_bias = tf.get_variable("marriageStatus_emb_bias", shape=(feature_conf_dict['marriageStatus'][0], 1), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        ...
        lbs_v_bias = tf.reshape(lbs_v_ori_bias, [-1, 1])
        ...
        age_v_bias = tf.reshape(age_v_ori_bias, [-1, 1])
        ...
        carrier_v_bias = tf.reshape(carrier_v_ori_bias, [-1, 1])
        ...
        consumptionability_v_bias = tf.reshape(consumptionability_v_ori_bias, [-1, 1])
        ...
        education_v_bias = tf.reshape(education_v_ori_bias, [-1, 1])
        ...
        gender_v_bias = tf.reshape(gender_v_ori_bias, [-1, 1])
        ...
        house_v_bias = tf.reshape(house_v_ori_bias, [-1, 1])
        ...
        os_v_bias = tf.reshape(os_v_ori_bias, [-1, 1])
        ...
        ct_v_bias = tf.reshape(ct_v_ori_bias, [-1, 1])
        ...
        appIdAction_v_bias = tf.reshape(tf.matmul(appidaction_val_p, appIdAction_v_ori_bias), [-1, 1])
        ...
        appIdInstall_v_bias = tf.reshape(tf.matmul(appIdInstall_val_p, appIdInstall_v_ori_bias), [-1, 1])
        ...
        marriagestatus_v_det_bias = tf.reshape(marriagestatus_v_ori_prod_bias, [-1, feature_conf_dict['marriageStatus'][0]])
        marriagestatus_v_bias = tf.reshape(tf.matmul(marriagestatus_val_p, marriagestatus_v_ori_bias), [-1, 1])
        ...
        interest1_v_det_bias = tf.reshape(interest1_v_ori_prod_bias, [-1, feature_conf_dict['interest1'][0] ])
        interest1_v_bias = tf.reshape(tf.matmul(interest1_val_p, interest1_v_ori_bias), [-1, 1])
        ...
        interest2_v_det_bias = tf.reshape(interest2_v_ori_prod_bias, [-1, feature_conf_dict['interest2'][0] ])
        interest2_v_bias = tf.reshape(tf.matmul(interest2_val_p, interest2_v_ori_bias), [-1, 1])
        ...
        interest3_v_det_bias = tf.reshape(interest3_v_ori_prod_bias, [-1, feature_conf_dict['interest3'][0]])
        interest3_v_bias = tf.reshape(tf.matmul(interest3_val_p, interest3_v_ori_bias), [-1, 1])
        ...
        interest4_v_det_bias = tf.reshape(interest4_v_ori_prod_bias, [-1, feature_conf_dict['interest4'][0]])
        interest4_v_bias = tf.reshape(tf.matmul(interest4_val_p, interest4_v_ori_bias), [-1, 1])
        ...
        interest5_v_det_bias = tf.reshape(interest5_v_ori_prod_bias, [-1, feature_conf_dict['interest5'][0]])
        interest5_v_bias = tf.reshape(tf.matmul(interest5_val_p, interest5_v_ori_bias), [-1, 1])

        kw1_v_ori_bias = tf.nn.embedding_lookup(kw1_emb_bias, kw1_index_p)
        kw1_v_bias = tf.reshape(tf.matmul(kw1_val_p, kw1_v_ori_bias), [-1, 1])

        kw2_v_ori_bias = tf.nn.embedding_lookup(kw2_emb_bias, kw2_index_p)
        kw2_v_bias = tf.reshape(tf.matmul(kw2_val_p, kw2_v_ori_bias), [-1, 1])

        kw3_v_ori_bias = tf.nn.embedding_lookup(kw3_emb_bias, kw3_index_p)
        kw3_v_bias = tf.reshape(tf.matmul(kw3_val_p, kw3_v_ori_bias), [-1, 1])

        topic1_v_ori_bias = tf.nn.embedding_lookup(topic1_emb_bias, topic1_index_p)
        topic1_v_bias = tf.reshape(tf.matmul(topic1_val_p, topic1_v_ori_bias), [-1, 1])

        topic2_v_ori_bias = tf.nn.embedding_lookup(topic2_emb_bias, topic2_index_p)
        topic2_v_bias = tf.reshape(tf.matmul(topic2_val_p, topic2_v_ori_bias), [-1, 1])

        topic3_v_ori_bias = tf.nn.embedding_lookup(topic3_emb_bias, topic3_index_p)
        topic3_v_bias = tf.reshape(tf.matmul(topic3_val_p, topic3_v_ori_bias), [-1, 1])
        ...
        kw1_v_det = tf.reshape(kw1_v_ori_prod, [-1, feature_conf_dict['kw1'][1] * emb_size])
        kw1_v = tf.reshape(tf.matmul(kw1_val_p, kw1_v_ori), [-1, low_emb_size])

        kw2_v_ori = tf.nn.embedding_lookup(kw2_emb, kw2_index_p)
        kw2_v_ori_prod = tf.multiply(tf.transpose(kw2_val_p, [0, 2, 1]), kw2_v_ori)
        kw2_v_det = tf.reshape(kw2_v_ori_prod, [-1, feature_conf_dict['kw2'][1] * emb_size])
        kw2_v = tf.reshape(tf.matmul(kw2_val_p, kw2_v_ori), [-1, low_emb_size])

        kw3_v_ori = tf.nn.embedding_lookup(kw3_emb, kw3_index_p)
        kw3_v_ori_prod = tf.multiply(tf.transpose(kw3_val_p, [0, 2, 1]), kw3_v_ori)
        kw3_v_det = tf.reshape(kw3_v_ori_prod, [-1, feature_conf_dict['kw3'][1] * emb_size])
        kw3_v = tf.reshape(tf.matmul(kw3_val_p, kw3_v_ori), [-1, low_emb_size])

        topic1_v_ori = tf.nn.embedding_lookup(topic1_emb, topic1_index_p)
        topic1_v_ori_prod = tf.multiply(tf.transpose(topic1_val_p, [0, 2, 1]), topic1_v_ori)
        topic1_v_det = tf.reshape(topic1_v_ori_prod, [-1, feature_conf_dict['topic1'][1] * emb_size])
        ...
        topic2_v_det = tf.reshape(topic2_v_ori_prod, [-1, feature_conf_dict['topic2'][1] * emb_size])
        ...
        topic3_v_det = tf.reshape(topic3_v_ori_prod, [-1, feature_conf_dict['topic3'][1] * emb_size])
        ...
        advertiserid_v_bias = tf.reshape(advertiserid_v_ori_bias, [-1, 1])
        ...
        campaignid_v_bias = tf.reshape(campaignid_v_ori_bias, [-1, 1])
        ...
        creativeid_v_bias = tf.reshape(creativeid_v_ori_bias, [-1, 1])
        ...
        adcategoryid_v_bias = tf.reshape(adcategoryid_v_ori_bias, [-1, 1])
        ...
        productid_v_bias = tf.reshape(productid_v_ori_bias, [-1, 1])
        ...
        producttype_v_bias = tf.reshape(producttype_v_ori_bias, [-1, 1])
        ...
        if graph_hyper_params['creativeSize_pro'] == 'min_max':
            ...
            creativesize_v_bias = tf.reshape(creativesize_v_ori_bias, [-1, 1])
        ...
        # init vector
        user_vector_fm = tf.concat([lbs_v, age_v, carrier_v, consumptionability_v, education_v, gender_v, house_v, os_v, ct_v,
                                marriagestatus_v_det, appIdAction_v, appIdInstall_v, interest1_v_det, interest2_v_det, interest3_v_det,
                                interest4_v_det, interest5_v_det, kw1_v_det, kw2_v_det, kw3_v_det, topic1_v_det, topic2_v_det, topic3_v_det], axis=-1)

        user_vector_fm_size = 7 + 4 + feature_conf_dict['marriageStatus'][0] + feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] +\
                              feature_conf_dict['interest4'][0] + feature_conf_dict['interest5'][0] + \
                              feature_conf_dict['kw1'][1] + feature_conf_dict['kw2'][1] + \
                              feature_conf_dict['kw3'][1] + feature_conf_dict['topic1'][1] + \
                              feature_conf_dict['topic2'][1] + feature_conf_dict['topic3'][1]
        ...
        uus = 13 + 4 + (feature_conf_dict['marriageStatus'][0] + feature_conf_dict['interest1'][0] + \
                        feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + \
                        feature_conf_dict['interest4'][0] + feature_conf_dict['interest5'][0])
        user_vector = tf.reshape(user_vector, [-1, uus, emb_size])
        ads = 7
        relation_mat = tf.matmul(user_vector, tf.transpose(ad_vector, perm=[0, 2, 1]))
        relation_mat_size = uus * ads
        relation_mat = tf.reshape(relation_mat, [-1, relation_mat_size])


        if graph_hyper_params['model'] == 'dmf':
            print 'dmf model !'
            user_vector_fm = tf.reshape(user_vector_fm, [-1, user_vector_fm_size, emb_size])

            # fm
            embeddings = tf.concat([user_vector_fm, ad_vector], axis=1)
            ...
            feature_bias = tf.reduce_sum(tf.concat([user_vector_bias, ad_vector_bias], axis=-1), axis=-1, keep_dims=True)
            qu_bias = tf.ones_like(feature_bias) * tf.get_variable("qu_bias", shape=(1, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)


            oout = tf.concat([relation_mat, y_second_order, feature_bias, qu_bias], axis=-1)
            final_size = uus*ads + emb_size + 1 + 2

            wf_1 = tf.get_variable("wf_1", shape=(final_size, final_size / 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            final_vec_wf_pre_1 = tf.matmul(oout, wf_1) + bf_1
            ...
            return pred_val, gmf_loss + regularization_loss, [pred_val, pre_pred_val, user_vector_fm, ad_vector]
    pass
