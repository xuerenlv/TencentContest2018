...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            # bias
            user_vector_bias = tf.concat([lbs_v_bias, age_v_bias, carrier_v_bias, consumptionability_v_bias, education_v_bias, gender_v_bias, house_v_bias, os_v_bias, ct_v_bias,
                 marriagestatus_v_det_bias, appIdAction_v_bias, appIdInstall_v_bias, interest1_v_det_bias, interest2_v_det_bias, interest3_v_det_bias,
                 interest4_v_det_bias, interest5_v_det_bias, kw1_v_bias, kw2_v_bias, kw3_v_bias, topic1_v_bias, topic2_v_bias, topic3_v_bias], axis=-1)
            # user_vector_biass = tf.tile(user_vector_bias, [1, ad_s]),
            # ad_vector_biass = tf.concat([
            #     tf.tile(advertiserid_v_bias, [1, uu_s]),
            #     tf.tile(campaignid_v_bias,[1, uu_s]),
            #     tf.tile(creativeid_v_bias,[1, uu_s]),
            #     tf.tile(adcategoryid_v_bias,[1, uu_s]),
            #     tf.tile(productid_v_bias,[1, uu_s]),
            #     tf.tile(producttype_v_bias,[1, uu_s]),
            #     tf.tile(creativesize_v_bias, [1, uu_s])
            # ], axis=-1)

            ad_vector_bias = tf.concat([advertiserid_v_bias, campaignid_v_bias, creativeid_v_bias, adcategoryid_v_bias,
                productid_v_bias, producttype_v_bias, creativesize_v_bias], axis=-1)
            # bias_mat = tf.reshape(tf.matmul(user_vector, tf.transpose(ad_vector_bias, perm=[0, 2, 1])), [-1, flat_size])

            user_vector_bias = tf.reshape(user_vector_bias, [-1, uu_s])
            ad_vector_bias = tf.reshape(ad_vector_bias, [-1, ad_s])



            relation_mat = tf.concat([relation_mat, user_vector_bias, ad_vector_bias], axis=-1)
