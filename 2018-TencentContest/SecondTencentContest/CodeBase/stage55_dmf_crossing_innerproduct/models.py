...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        kw1_v = tf.reshape(tf.matmul(kw1_val_p, kw1_v_ori), [-1, low_emb_size]) #/ (tf.reduce_sum(tf.cast(tf.cast(kw1_val_p, dtype=tf.bool), dtype=tf.float32), axis=-1) + 1e-6)

        kw2_v_ori = tf.nn.embedding_lookup(kw2_emb, kw2_index_p)
        kw2_v = tf.reshape(tf.matmul(kw2_val_p, kw2_v_ori), [-1, low_emb_size]) #/ (tf.reduce_sum(tf.cast(tf.cast(kw2_val_p, dtype=tf.bool), dtype=tf.float32), axis=-1) + 1e-6)

        kw3_v_ori = tf.nn.embedding_lookup(kw3_emb, kw3_index_p)
        kw3_v = tf.reshape(tf.matmul(kw3_val_p, kw3_v_ori), [-1, low_emb_size])#/ (tf.reduce_sum(tf.cast(tf.cast(kw3_val_p, dtype=tf.bool), dtype=tf.float32), axis=-1) + 1e-6)
        ...
        topic2_v = tf.reshape(tf.matmul(topic2_val_p, topic2_v_ori), [-1, emb_size])#/ (tf.reduce_sum(tf.cast(tf.cast(topic2_val_p, dtype=tf.bool), dtype=tf.float32), axis=-1) + 1e-6)

        topic3_v_ori = tf.nn.embedding_lookup(topic3_emb, topic3_index_p)
        topic3_v = tf.reshape(tf.matmul(topic3_val_p, topic3_v_ori), [-1, emb_size])#/ (tf.reduce_sum(tf.cast(tf.cast(topic3_val_p, dtype=tf.bool), dtype=tf.float32), axis=-1) + 1e-6)
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            # for i in range(uu_s):
            #     for j in range(ad_s):
            #         mat_concat_list.append(tf.reduce_sum(tf.abs(user_vector_lists[i] - ad_vector_lists[j]), axis=-1))
            # abs_relation_mat = tf.concat(mat_concat_list, axis=-1)

            user_vector_ss = tf.tile(user_vector, [1, ad_s, 1])
            ad_vector_lists = tf.split(ad_vector, [1 for i in range(ad_s)], axis=1)
            ad_vector_lists_ss = []
            for j in range(ad_s):
                ad_vector_lists_ss.append(tf.tile(ad_vector_lists[i], [1, uu_s, 1]))
            ad_vector_ss = tf.concat(ad_vector_lists_ss, axis=1)

            inner_mat = user_vector_ss * ad_vector_ss
            abs_relation_mat = tf.abs(user_vector_ss - ad_vector_ss)

            flat_size = uu_s * ad_s * emb_size
            print 'uu_s, ad_s: ', uu_s, ad_s
            print 'flat_size', flat_size
            relation_mat = tf.concat([tf.reshape(inner_mat, [-1, flat_size]), tf.reshape(abs_relation_mat, [-1, flat_size])], axis=-1)

            # final_relation_mat = tf.concat([relation_mat, abs_relation_mat], axis=-1)
            final_relation_mat = relation_mat
            final_size = 2400
            wf_1 = tf.get_variable("wf_1", shape=(flat_size*2, final_size/4), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_1 = tf.get_variable("bf_1", shape=[final_size/4], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_pre_1 = tf.matmul(final_relation_mat, wf_1) + bf_1
            final_vec_wf_1 = tf.nn.relu(final_vec_wf_pre_1)

            wf_2 = tf.get_variable("wf_2", shape=(final_size/4, final_size / 8), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_2 = tf.get_variable("bf_2", shape=[final_size / 8], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_2 = tf.matmul(final_vec_wf_1, wf_2) + bf_2
            final_vec_wf_2 = tf.nn.relu(final_vec_wf_2)

            wf_3 = tf.get_variable("wf_3", shape=(final_size / 8, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
