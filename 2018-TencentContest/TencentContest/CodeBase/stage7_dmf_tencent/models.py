...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        cross_b1 = tf.get_variable("cross_b1", shape=[cross_emb_size*4], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        cross_net1 = tf.nn.relu(tf.reshape(tf.matmul(all_cross_module_val, cross_emb_ori), [-1, cross_emb_size*4]) + cross_b1)
        # cross_net1 = tf.nn.relu(tf.reshape(tf.matmul(all_cross_module_val, cross_emb_ori), [-1, cross_emb_size*2]))

        wwf = tf.get_variable("wwf", shape=(cross_emb_size*4, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
