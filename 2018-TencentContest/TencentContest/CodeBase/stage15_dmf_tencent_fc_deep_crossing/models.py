...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            # final_vec_wf_1 = parametric_relu(final_vec_wf_pre_1, 'first_layer_alpha')
            ...
            # final_vec_wf_2 = parametric_relu(final_vec_wf_2, 'second_layer_alpha')

            wf_3 = tf.get_variable("wf_3", shape=(final_size / 4, final_size / 8), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_3 = tf.get_variable("bf_3", shape=[final_size / 8], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_3 = tf.matmul(final_vec_wf_2, wf_3) + bf_3
            final_vec_wf_3 = tf.nn.relu(final_vec_wf_3)

            wf_4 = tf.get_variable("wf_4", shape=(final_size / 8, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_4 = tf.get_variable("bf_4", shape=[2], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_4 = tf.matmul(final_vec_wf_3, wf_4) + bf_4

            pre_pred_val_deep = tf.split(tf.nn.softmax(final_vec_wf_4), [1, 1], axis=1, name='pred')[0]
