...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            relation_mat_u = tf.matmul(user_vector, tf.transpose(user_vector, perm=[0, 2, 1]))
            relation_mat_a = tf.matmul(ad_vector, tf.transpose(ad_vector, perm=[0, 2, 1]))


            user_feature_num = 14 + 4 + feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0] + feature_conf_dict['interest5'][0]
            ad_feature_num = 7

            relation_mat = tf.reshape(relation_mat, [-1, user_feature_num * ad_feature_num])
            relation_mat_u = tf.reshape(relation_mat_u, [-1, user_feature_num * user_feature_num])
            relation_mat_a = tf.reshape(relation_mat_a, [-1, ad_feature_num * ad_feature_num])

            relation_mat = tf.concat([relation_mat, relation_mat_u, relation_mat_a], axis=-1)
            flat_size = user_feature_num * ad_feature_num +\
                        user_feature_num * user_feature_num +\
                        ad_feature_num * ad_feature_num
            print 'flat_size', flat_size
            wf_1 = tf.get_variable("wf_1", shape=(flat_size, 1500), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_1 = tf.get_variable("bf_1", shape=[1500], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_pre_1 = tf.matmul(relation_mat, wf_1) + bf_1
            final_vec_wf_1 = tf.nn.relu(final_vec_wf_pre_1)

            wf_2 = tf.get_variable("wf_2", shape=(1500, 128), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_2 = tf.get_variable("bf_2", shape=[128], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_2 = tf.matmul(final_vec_wf_1, wf_2) + bf_2
            final_vec_wf_2 = tf.nn.relu(final_vec_wf_2)

            wf_3 = tf.get_variable("wf_3", shape=(128, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            final_vec_wf_2 = tf.matmul(final_vec_wf_2, wf_3) + bf_3
