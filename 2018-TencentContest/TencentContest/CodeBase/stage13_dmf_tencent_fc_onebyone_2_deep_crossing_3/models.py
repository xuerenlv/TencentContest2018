...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        user_val = tf.concat([
            ...
            tf.expand_dims(tf.cast(tf.cast(lbs_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(age_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(carrier_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(consumptionability_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(education_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(gender_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(house_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(os_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(ct_p, dtype=tf.bool), dtype=tf.float32), axis=1),
            tf.expand_dims(tf.cast(tf.cast(marriagestatus_p, dtype=tf.bool), dtype=tf.float32), axis=1),
        ...
        # cross_emb_ori = tf.concat([user_v_ori * ad_v_ori, tf.abs(user_v_ori - ad_v_ori)], axis=-1)
        ...
        res_emb_size = cross_emb_size*2
        wwwf_res1 = tf.get_variable("wwwf_res1", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res1 = tf.get_variable("wwbf_res1", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res1 = tf.matmul(cross_net1, wwwf_res1) + wwbf_res1
        final_vec_wwwf_res1 = tf.nn.relu(final_vec_wwwf_pre_res1)

        wwwf_res2 = tf.get_variable("wwwf_res2", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res2 = tf.get_variable("wwbf_res2", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res2 = tf.matmul(final_vec_wwwf_res1, wwwf_res2) + wwbf_res2
        final_vec_wwwf_res2 = tf.nn.relu(final_vec_wwwf_pre_res2)

        # res unit 1
        cross_net1 = cross_net1 + final_vec_wwwf_res2

        wwwf_res3 = tf.get_variable("wwwf_res3", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res3 = tf.get_variable("wwbf_res3", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res3 = tf.matmul(cross_net1, wwwf_res3) + wwbf_res3
        final_vec_wwwf_res3 = tf.nn.relu(final_vec_wwwf_pre_res3)

        wwwf_res4 = tf.get_variable("wwwf_res4", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res4 = tf.get_variable("wwbf_res4", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res4 = tf.matmul(final_vec_wwwf_res3, wwwf_res4) + wwbf_res4
        final_vec_wwwf_res4 = tf.nn.relu(final_vec_wwwf_pre_res4)

        # res unit 2
        cross_net1 = cross_net1 + final_vec_wwwf_res4

        wwwf_res5 = tf.get_variable("wwwf_res5", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res5 = tf.get_variable("wwbf_res5", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res5 = tf.matmul(cross_net1, wwwf_res5) + wwbf_res5
        final_vec_wwwf_res5 = tf.nn.relu(final_vec_wwwf_pre_res5)

        wwwf_res6 = tf.get_variable("wwwf_res6", shape=(res_emb_size, res_emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        wwbf_res6 = tf.get_variable("wwbf_res6", shape=[res_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        final_vec_wwwf_pre_res6 = tf.matmul(final_vec_wwwf_res5, wwwf_res6) + wwbf_res6
        final_vec_wwwf_res6 = tf.nn.relu(final_vec_wwwf_pre_res6)

        cross_net1 = cross_net1 + final_vec_wwwf_res6
        ...
        def get_variable(shape, name):
            return tf.get_variable(name, shape=shape, initializer=tf.contrib.layers.xavier_initializer(),
                                   dtype=tf.float32, regularizer=regularizer)

        def _conv_weight(isz, osz, name):
            return get_variable([2, 2, isz, osz], name+'_w'), get_variable([osz], name+'_b')

        def _conv_layer(input, P):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            user_vector = tf.reshape(user_vector, [-1, 14 + 4 + (feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0]
                            + feature_conf_dict['interest5'][0]), emb_size])
            ad_vector = tf.reshape(ad_vector, [-1, 7, ad_emb_size])

            user_and_ad_vec = tf.concat([user_vector, ad_vector], axis=1)
            user_and_ad_vec_2 = tf.concat([user_vector, ad_vector], axis=1)
            relation_mat = tf.matmul(user_and_ad_vec, tf.transpose(user_and_ad_vec_2, perm=[0, 2, 1]))


            uu_s = 14 + 4 + feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0] + feature_conf_dict['interest5'][0]
            ad_s = 7
            flat_size = (uu_s + ad_s) * (uu_s + ad_s)

            print 'uu_s, ad_s: ', uu_s, ad_s
            print 'flat_size', flat_size
            relation_mat = tf.reshape(relation_mat, [-1, flat_size])

            # wf_relation_mat = tf.get_variable("wf_relation_mat", shape=(1, flat_size ), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # bf_relation_mat = tf.get_variable("bf_relation_mat", shape=[flat_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # relation_mat = tf.nn.relu(tf.multiply(wf_relation_mat, relation_mat) + bf_relation_mat)
            ...
            wf_3 = tf.get_variable("wf_3", shape=(final_size / 4, final_size / 6), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_3 = tf.get_variable("bf_3", shape=[final_size / 6], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_3 = tf.matmul(final_vec_wf_2, wf_3) + bf_3


            ### deep part
            deep_size = (uu_s + ad_s) * emb_size
            deep_final_size = 1200
            user_and_ad_vec = tf.reshape(user_and_ad_vec, [-1, deep_size])
            wf_deep1 = tf.get_variable("wf_deep1", shape=(deep_size, deep_final_size/2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_deep1 = tf.get_variable("bf_deep1", shape=[deep_final_size/2], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_deep1 = tf.matmul(user_and_ad_vec, wf_deep1) + bf_deep1
            final_vec_deep1 = tf.nn.relu(final_vec_deep1)

            wf_deep2 = tf.get_variable("wf_deep2", shape=(deep_final_size / 2, deep_final_size / 4), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32,regularizer=regularizer)
            bf_deep2 = tf.get_variable("bf_deep2", shape=[deep_final_size / 4], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_deep2 = tf.matmul(final_vec_deep1, wf_deep2) + bf_deep2
            final_vec_deep2 = tf.nn.relu(final_vec_deep2)

            wf_deep3 = tf.get_variable("wf_deep3", shape=(deep_final_size / 4, deep_final_size / 6),initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32,regularizer=regularizer)
            bf_deep3 = tf.get_variable("bf_deep3", shape=[deep_final_size / 6], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_deep3 = tf.matmul(final_vec_deep2, wf_deep3) + bf_deep3

            ###
            final_vec_wffff = tf.concat([final_vec_wf_3, final_vec_deep3], axis=-1)
            wffff = tf.get_variable("wffff", shape=(final_size / 6 + deep_final_size / 6, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bffff = tf.get_variable("bffff", shape=[2], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_pre_pre = tf.matmul(final_vec_wffff, wffff) + bffff

            pre_pred_val_deep = tf.split(tf.nn.softmax(final_vec_wf_pre_pre), [1, 1], axis=1, name='pred')[0]
