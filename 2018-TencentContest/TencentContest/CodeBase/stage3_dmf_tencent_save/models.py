...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        user_vector_size = emb_size * 19 + 4 * low_emb_size
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            u_w2 = tf.get_variable("u_w2", shape=(user_vector_size, 1000), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            u_b2 = tf.get_variable("u_b2", shape=[1000], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            v_w2 = tf.get_variable("v_w2", shape=(ad_vector_size, 1000), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            v_b2 = tf.get_variable("v_b2", shape=[1000], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            if graph_hyper_params['use_kernal']:
                ...
                final_vec = tf.concat([tf.square(net_u_final - net_v_final), net_u_final * net_v_final], axis=-1)
                # final_vec = tf.nn.dropout(final_vec, dropout_p[0])
                wf = tf.get_variable("wf", shape=(2000, 500), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                bf = tf.get_variable("bf", shape=[500], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                final_vec_wf_pre = tf.matmul(final_vec, wf) + bf
                if graph_hyper_params['use_bn']:
                    final_vec_wf_pre = batch_norm_layer(final_vec_wf_pre, train_p, 0.995, 'final_vec_wf_pre')
                final_vec_wf = tf.nn.relu(final_vec_wf_pre)
                final_vec_wf = tf.nn.dropout(tf.nn.relu(final_vec_wf), dropout_p[0])
                ...
                final_vec_wf_2 = tf.matmul(final_vec_wf, wf2) + bf2

                # wf3 = tf.get_variable("wf3", shape=(128, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                # bf3 = tf.get_variable("bf3", shape=[2], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                # final_vec_wf_3 = tf.matmul(tf.nn.relu(final_vec_wf_2), wf3) + bf3
