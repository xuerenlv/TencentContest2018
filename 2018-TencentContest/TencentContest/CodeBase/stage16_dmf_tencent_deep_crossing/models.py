...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            # single func
            # wf_user_and_ad = tf.get_variable("wf_user_and_ad", shape=(uu_s + ad_s, emb_size), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # single_func = tf.reduce_sum(user_and_ad_vec*wf_user_and_ad, axis=-1)

            # flat_size += uu_s + ad_s
            # relation_mat = tf.concat([relation_mat, single_func], axis=-1)
