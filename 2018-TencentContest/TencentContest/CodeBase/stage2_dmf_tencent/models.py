...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            net_u_2_pre_asf = tf.matmul(net_u_1, u_w2) + u_b2
            if graph_hyper_params['use_bn']:
            ...
            net_u_3_pre = tf.matmul(net_u_2, u_w3) + u_b3
            # if graph_hyper_params['use_bn']:
            ...
            net_v_3_pre = tf.matmul(net_v_2, v_w3) + v_b3
            # if graph_hyper_params['use_bn']:
            ...
            net_u_final = net_u_2_pre_asf + net_u_3_pre
            net_v_final = net_v_2_pre_asf + net_v_3_pre
            # net_u_final = net_u_2_pre
            # net_v_final = net_v_2_pre

            if graph_hyper_params['use_kernal']:
                final_vec = tf.concat([tf.abs(net_u_final-net_v_final), net_u_final*net_v_final], axis = -1)
                final_vec = tf.nn.dropout(final_vec, dropout_p[0])
                wf = tf.get_variable("wf", shape=(1000, 1), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                bf = tf.get_variable("bf", shape=[1], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
                with tf.name_scope("final"):
                    pred_val = tf.nn.sigmoid(tf.matmul(final_vec,wf)+bf, name='pred')
            else:
