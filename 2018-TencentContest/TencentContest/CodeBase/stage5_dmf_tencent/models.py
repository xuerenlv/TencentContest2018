...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        # all_cross_module_index = tf.tile(user_index, [1, 7]) + ad_cross_index * user_all_len
        # all_cross_module_val = tf.tile(user_val, [1, 1, 7])
        # cross_v_ori = tf.nn.embedding_lookup(cross_module_emb, all_cross_module_index)
        # cross_b1 = tf.get_variable("cross_b1", shape=[cross_emb_size], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
        # cross_net1 = tf.reshape(tf.matmul(all_cross_module_val, cross_v_ori), [-1, cross_emb_size]) + cross_b1
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            if graph_hyper_params['use_kernal']:
                ...
                final_vec_wf_2 = tf.matmul(final_vec_1, wf2) + bf2 + cross_net1
