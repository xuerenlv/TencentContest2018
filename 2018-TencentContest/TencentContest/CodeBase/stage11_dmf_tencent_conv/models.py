...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            relation_mat = tf.expand_dims(tf.matmul(ad_vector, tf.transpose(user_vector, perm=[0, 2, 1])), axis=-1)


            nc = [32, 32, 32, 32, 32, 32]
            iszs = [1] + nc[:-1]
            oszs = nc
            P = []
            ii = 0
            ...
            input = tf.reshape(input, [32, -1])

            ll = input.get_shape().as_list()
            W = get_variable([ll[-1], 2], name='final_w')
            b = get_variable([2], name='final_b')
            output_layer = tf.matmul(input, W) + b
            ...
            return pred_val, gmf_loss + regularization_loss, [pre_pred_val, user_vector, ad_vector]
