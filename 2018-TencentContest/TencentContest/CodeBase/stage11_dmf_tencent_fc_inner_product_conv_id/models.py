...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        # user
        # uid_v_ori = tf.nn.embedding_lookup(uid_emb, uid_p)
        # uid_v = tf.reshape(uid_v_ori, [-1, emb_size])
        ...
        # ad
        aid_v_ori =  tf.nn.embedding_lookup(aid_emb, aid_p)
        aid_v = tf.reshape(aid_v_ori, [-1, ad_emb_size])
        ...
        if graph_hyper_params['creativeSize_pro'] == 'min_max':
            ...
            ad_vector_size = ad_emb_size * 7 + 1 # for dmf
            ...
            ad_vector_size = ad_emb_size * 8  # for dmf
        else:
        ...
        ad_vector = tf.concat([aid_v, advertiserid_v, campaignid_v, creativeid_v, adcategoryid_v, productid_v, producttype_v, creativesize_v], axis=-1)
        ...
        def _conv_weight(isz, osz, name):
            return get_variable([3, 3, isz, osz], name+'_w'), get_variable([osz], name+'_b')

        def _conv_layer(input, P):
            conv = tf.nn.conv2d(input, P[0], strides=[1, 3, 2, 1], padding='SAME')
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            user_vector = tf.reshape(user_vector, [-1, 14 + 4 + (feature_conf_dict['interest1'][0] + feature_conf_dict['interest2'][0] + feature_conf_dict['interest3'][0] + feature_conf_dict['interest4'][0]
                            + feature_conf_dict['interest5'][0]), emb_size])
            ad_vector = tf.reshape(ad_vector, [-1, 8, ad_emb_size])

            # relation_mat = tf.matmul(ad_vector, tf.transpose(user_vector, perm=[0, 2, 1]))
            ...
            user_vector_lists = tf.split(user_vector, [1 for i in range(uu_s)], axis=1)
            ad_vector_lists = tf.split(ad_vector, [1 for i in range(ad_s)], axis=1)
            mat_concat_list = []
            for i in range(uu_s):
                for j in range(ad_s):
                    mat_concat_list.append(user_vector_lists[i] * ad_vector_lists[j])

            # print 'uu_s, ad_s: ', uu_s, ad_s
            # print 'flat_size', flat_size
            relation_mat = tf.reshape(tf.concat(mat_concat_list, axis=1), [-1, uu_s, ad_s, emb_size])
            ...
            for isz, osz in zip(iszs, oszs):
                P.append(_conv_weight(isz, osz, name=str(ii) + '_' + str(isz) + '_' + str(osz)))
                ii += 1

            layer = []
            input = relation_mat
            for p in P:
                layer.append(_conv_layer(input, p))
                input = layer[-1]


            # ooo_out = tf.reshape(input, [-1, 6 * 7 * 32])
            flat_size = 15 * 1 * 25
            # relation_mat= tf.concat(mat_concat_list, axis=-1)
            print 'uu_s, ad_s: ', uu_s, ad_s
            print 'flat_size', flat_size
            input = tf.reshape(input, [-1, flat_size])

            # wf_1 = tf.get_variable("wf_1", shape=(flat_size, 128), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # bf_1 = tf.get_variable("bf_1", shape=[128], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # final_vec_wf_pre_1 = tf.matmul(input, wf_1) + bf_1
            # final_vec_wf_1 = tf.nn.relu(final_vec_wf_pre_1)

            # wf_2 = tf.get_variable("wf_2", shape=(1500, 300), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # bf_2 = tf.get_variable("bf_2", shape=[300], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            # final_vec_wf_2 = tf.matmul(final_vec_wf_1, wf_2) + bf_2
            # final_vec_wf_2 = tf.nn.relu(final_vec_wf_2)

            wf_3 = tf.get_variable("wf_3", shape=(flat_size, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            final_vec_wf_2 = tf.matmul(input, wf_3) + bf_3

            final_vec_wf_2 = final_vec_wf_2
