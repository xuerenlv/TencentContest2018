...
def inference(uid_p, lbs_p, age_p, carrier_p, consumptionability_p, education_p, gender_p, house_p,
    ...
    with tf.variable_scope("OutAll"):
        ...
        if graph_hyper_params['model'] == 'dmf':
            ...
            wf_1 = tf.get_variable("wf_1", shape=(flat_size, flat_size/2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_1 = tf.get_variable("bf_1", shape=[flat_size/2], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_pre_1 = tf.matmul(relation_mat, wf_1) + bf_1
            final_vec_wf_1 = tf.nn.relu(final_vec_wf_pre_1)

            wf_2 = tf.get_variable("wf_2", shape=(flat_size/2, flat_size / 4), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            bf_2 = tf.get_variable("bf_2", shape=[flat_size / 4], initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            final_vec_wf_2 = tf.matmul(final_vec_wf_1, wf_2) + bf_2
            final_vec_wf_2 = tf.nn.relu(final_vec_wf_2)

            wf_3 = tf.get_variable("wf_3", shape=(flat_size / 4, 2), initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32, regularizer=regularizer)
            ...
            gmf_loss = tf.reduce_mean(-true_label * tf.log(pre_pred_val_deep + 1e-6) - graph_hyper_params['pnw'] * (1.0 - true_label) * tf.log(1.0 - pre_pred_val_deep + 1e-6))
